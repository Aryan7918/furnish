(function () {
    ...
    let navActive = () => {
        ...
        isUrl = isUrl.split(".test/").slice(1);
        if (isUrl == "") {
            let navLink = document.getElementsByClassName("nav-link");
            if (navLink[0].getAttribute("data-link") == "home") {
                navLink[0].parentElement.classList.add("active");
            }
        } else {
            let navLink = document.getElementsByClassName("nav-link");
            for (let i = 0; i < navLink.length; i++) {
                if (navLink[i].getAttribute("data-link") == isUrl) {
